import json
import os
from typing import Dict, Any, List, Optional

PROFILE_KEYS: List[str] = ["from_mail", "app_password", "send_name", "send_team"]

def profile_path_default() -> str:
    # 프로젝트 루트의 config.json을 기본으로 사용
    return os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "..", "config.json"))

def profile_check(profile_path: str) -> bool:
    """프로필 유무 확인(True/False)"""
    if not os.path.exists(profile_path):
        return False

    with open(profile_path, "r", encoding="utf-8") as f:
        cfg = json.load(f)

    profiles = cfg.get("profiles", [])
    if not profiles:
        return False

    p = profiles[0] or {}
    return all(p.get(k) for k in PROFILE_KEYS)

def load_first_profile(profile_path: str) -> Optional[dict]:
    if not os.path.exists(profile_path):
        return None
    with open(profile_path, "r", encoding="utf-8") as f:
        cfg = json.load(f)
    profiles = cfg.get("profiles", [])
    if not profiles:
        return None
    return profiles[0] or None

def create_profile(state: dict | None = None) -> dict:
    """프로필 생성에 필요한 것들만 입력으로 받음"""
    LABELS = {
        "from_mail": "발신자 이메일",
        "app_password": "앱 비밀번호",
        "send_name": "발신자명",
        "send_team": "발신자 소속",
    }

    state = state or {}
    profile: Dict[str, Any] = {}

    for k in PROFILE_KEYS:
        if state.get(k):
            profile[k] = state[k]
            continue
        profile[k] = input(f"{LABELS[k]}> ").strip()

    return profile

def save_profile(profile_path: str, profile: dict) -> None:
    """프로필 저장"""
    cfg: Dict[str, Any] = {}
    if os.path.exists(profile_path):
        with open(profile_path, "r", encoding="utf-8") as f:
            cfg = json.load(f)

    cfg["profiles"] = [{k: profile.get(k, "") for k in PROFILE_KEYS}]

    with open(profile_path, "w", encoding="utf-8") as f:
        json.dump(cfg, f, ensure_ascii=False, indent=2)
