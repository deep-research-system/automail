...
from app.prompt import input_extract_prompt
...
class Extractdata(BaseModel):
    ...
    receive_name: Optional[str] = Field(description="수신자명", default=None)
    to_mail: Optional[str] = Field(description="수신자 메일", default=None)
    mail_type: Optional[MailType] = Field(description="메일 타입(견적서/보고서/일정/일반)", default=None)
...
def extract_llm(user_request: str) -> dict:
    ...
    out = data.model_dump(exclude_none=True)  # None 값 제거

    # user_request는 원본을 항상 들고 가는 게 편하니 강제로 세팅
